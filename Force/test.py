...
class Datasets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(parts) == 4:  # "0.jpg Fx Fy Fz"
            ...
            img_candidates = [
                os.path.join(self.data_path, 'frames', image_name),
...
class Trainer():
    ...
    def test(self):
        test_loader = DataLoader(dataset=self.test_datasets, batch_size=self.config.BATCH_SIZE,
                                 num_workers=self.config.NUM_WORKS, drop_last=False)
        self.model.eval()
    ...
    def load(self):
        model_path = '/media/disk1/ylt/Force/session_GLAS_002/sensor05/checkpoints.pt'
        print('\nLoading ...\n', model_path)
...
if __name__ == "__main__":
    ...
    # args.DATA_PATH = 'session_WET_TILE_010/'  # 数据根目录
    # args.save_path = 'session_WET_TILE_010/'
   
    #  + args.sensor + '/'
    args.load_path = args.save_path
    args.Stiffness_TRAIN_FLIST = os.path.join('session1', args.sensor, 'train_with_force.flist')
    args.Stiffness_VAL_FLIST = os.path.join('session1', args.sensor, 'val_with_force.flist')
    args.Stiffness_TEST_FLIST = os.path.join('session1', args.sensor, 'test_with_force.flist')
