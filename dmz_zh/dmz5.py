...
PITCH_UP_THRESHOLD     = 3.0    # 上坡俯仰角阈值（°）
PITCH_DOWN_THRESHOLD   = -3.0   # 下坡俯仰角阈值（°）
last_pitch = None
i = 0
# 全局状态
...
def detect_slope(pitch, wx):
    ...
    if i==100:
        last_pitch = pitch
        i=0
    if delta > PITCH_UP_THRESHOLD and abs(wx)<25:
        return "下坡"
    elif delta < PITCH_DOWN_THRESHOLD and abs(wx)<25:
        return "上坡"
    else:
        return "平路"
...
def detect_step_with_confirmation(current_h,wx):
    ...
    # 第一次检测到突变
    if pending_step is None and abs(wx)>25 :
        ...
        if pending_height_ref is None or pending_step_time is None:
            print("[STEP] 高度基准丢失，强制重置")
            pending_step       = None
            pending_step_time  = None
            pending_height_ref = None
            detect_step_with_confirmation.last_h = current_h
        ...
        while True:
            if ser.in_waiting >= 10:
                ...
                if t[0] == 0x52:  # 陀螺仪帧
                    ...
                    print(pitch)
                    slope_state = detect_slope(pitch,current_wx)

                elif t[0] == 0x56:  # 高度帧
                    h = parse_height_from_pressure_frame(frame)
                    step_state = detect_step_with_confirmation(h,current_wx)
