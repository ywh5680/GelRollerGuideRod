...
class calibration:
    def __init__(self):
        self.BallRad= 16/2 #4.76/2 #mm
        self.Pixmm = 0.0727 # .10577     #.10577 #4.76/100 #0.0806 * 1.5 mm/pixel
        self.ratio = 1/2.
...
#        mask = cv2.dilate(mask, self.kernal4, iterations=1)
    ...
    def contact_detection(self,raw_image, ref, marker_mask):
        ...
        contact_mask = (diff_img> 20).astype(np.uint8)*(1-marker_mask)
...
if __name__=="__main__":
   ...
   # cv2.waitKey(0)
    has_marke = True 
    img_list = glob.glob("tong4/*.jpg")
    
    for name in img_list:
        if name.split('\\')[1] == '0.jpg':
            continue
        print(name)
        img = cv2.imread(name)
        # img = img[:,220:940]
