...
def process_imu(msg_system):
    ...
    angle_window = deque(maxlen=40)   # 角度滑动窗口（可调整）
    height_window = deque(maxlen=20)  # 高度滑动窗口（可调整）
    ...
    while True:
        try:
            if ser.in_waiting >= 10:
                ...
                if last_pitch and last_height:
                    angle_window.append((now, last_pitch))
                    height_window.append((now, last_height))

            if len(angle_window) < 2 or len(height_window) < 2:
                time.sleep(0.005)
                continue

            # 解析角度数据
            angle_times, pitches = zip(*angle_window)
            angle_change = max(pitches) - min(pitches)
            angle_duration = angle_times[-1] - angle_times[0] or 1e-6

            # 解析高度数据
            height_times, heights = zip(*height_window)
            height_rate = abs(heights[-1] - heights[0]) / (height_times[-1] - height_times[0] or 1e-6)
            height_jump = abs(heights[-1] - heights[0])
